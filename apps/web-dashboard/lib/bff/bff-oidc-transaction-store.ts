...
export function setOidcTransactionRedisClientFactoryForTesting(factory: ((url: string) => MinimalRedisClient) | null): void {
...
async function requireRedis(): Promise<MinimalRedisClient> {
  const url = redisUrl();
  if (!url) throw new OidcTransactionStoreUnavailableError("ORDERPILOT_BFF_REDIS_URL (or REDIS_URL) is required for OIDC transactions");
  if (!redisClient) {
    redisClient = redisClientFactoryForTesting ? redisClientFactoryForTesting(url) : (createClient({ url, socket: { reconnectStrategy: false, connectTimeout: 5000 } }) as unknown as MinimalRedisClient);
    redisClient.on?.("error", () => undefined);
  }
  const client = redisClient;
  if (!client.isOpen) {
    if (!redisConnectInFlight) {
      redisConnectInFlight = Promise.resolve(client.connect()).then(() => undefined).finally(() => { redisConnectInFlight = null; });
    }
    try { await redisConnectInFlight; } catch { throw new OidcTransactionStoreUnavailableError("OIDC Redis transaction backend is not reachable"); }
  }
  return client;
}

function validTransaction(tx: OidcAuthorizationTransaction, nowEpochSec: number): boolean {
  return STATE_VALUE.test(tx.state) && STATE_VALUE.test(tx.nonce) && STATE_VALUE.test(tx.pkceVerifier) &&
    TEXT_VALUE.test(tx.redirectUri) && TEXT_VALUE.test(tx.issuer) && TEXT_VALUE.test(tx.audience) &&
    Number.isSafeInteger(tx.createdAtEpochSec) && Number.isSafeInteger(tx.expiresAtEpochSec) &&
    tx.createdAtEpochSec > 0 && tx.createdAtEpochSec <= nowEpochSec &&
    tx.expiresAtEpochSec > nowEpochSec && tx.expiresAtEpochSec - tx.createdAtEpochSec <= MAX_TRANSACTION_AGE_SECONDS;
}
...
export async function persistOidcAuthorizationTransaction(transaction: OidcAuthorizationTransaction): Promise<void> {
  const now = Math.floor(Date.now() / 1000);
  if (!validTransaction(transaction, now)) throw new OidcTransactionStoreUnavailableError("OIDC authorization transaction is invalid");
  ...
  const stored = await redis.set(`${TRANSACTION_KEY_PREFIX}${transaction.state}`, JSON.stringify(transaction), { EX: ttl, NX: true });
  if (stored !== "OK") throw new OidcTransactionStoreUnavailableError("OIDC authorization transaction could not be stored");
}

export async function consumeOidcAuthorizationTransaction(state: string | undefined): Promise<OidcAuthorizationTransaction | null> {
