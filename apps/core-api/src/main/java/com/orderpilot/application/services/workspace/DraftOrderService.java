...
public class DraftOrderService {
  ...
  public DraftOrder createFromValidation(UUID validationRunId) {
    ...
    DraftOrder order = orderRepository.save(new DraftOrder(tenantId, "DO-" + clock.instant().toEpochMilli(), customerId, run.getExtractionResultId(), validationRunId, null, status, null, null, clock.instant()));
    ...
    for (ExtractedLineItem line : lineRepository.findByTenantIdAndExtractionResultId(tenantId, run.getExtractionResultId())) {
      ...
      if (ol.getLineTotal() != null) subtotal = subtotal.add(ol.getLineTotal());
    }
    order.setTotals(subtotal, BigDecimal.ZERO, subtotal, null, clock.instant()); actionService.record(null, "DRAFT_ORDER", order.getId(), "ORDER_DRAFT_CREATED", "Internal draft order created from validation run without inventory reservation", "{\"validationRunId\":\"" + validationRunId + "\"}"); return orderRepository.save(order);
  }
