...
public class DraftQuoteService {
  ...
  public DraftQuote createFromValidation(UUID validationRunId, UUID sourceExceptionCaseId) {
    ...
    BigDecimal subtotal = BigDecimal.ZERO; String currency = null;
    for (ExtractedLineItem line : lineRepository.findByTenantIdAndExtractionResultId(tenantId, run.getExtractionResultId())) {
      ...
      if (ql.getLineTotal() != null) subtotal = subtotal.add(ql.getLineTotal()); if (currency == null && price != null) currency = price.getCurrency();
    }
    quote.setTotals(subtotal, BigDecimal.ZERO, subtotal, null, clock.instant()); actionService.record(null, "DRAFT_QUOTE", quote.getId(), "QUOTE_DRAFT_CREATED", "Internal draft quote created from validation run", "{\"validationRunId\":\"" + validationRunId + "\"}"); return quoteRepository.save(quote);
  }
