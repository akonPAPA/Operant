...
public interface ExceptionCaseRepository extends JpaRepository<ExceptionCase, UUID> {
  List<ExceptionCase> findByTenantIdOrderByCreatedAtDesc(UUID tenantId);
  List<ExceptionCase> findByTenantIdOrderByCreatedAtDesc(UUID tenantId, Pageable pageable); Optional<ExceptionCase> findByIdAndTenantId(UUID id, UUID tenantId); Optional<ExceptionCase> findFirstByTenantIdAndValidationRunIdOrderByCreatedAtDesc(UUID tenantId, UUID validationRunId); List<ExceptionCase> findByTenantIdAndStatusOrderByCreatedAtDesc(UUID tenantId, String status); long countByTenantIdAndStatus(UUID tenantId, String status); long countByTenantId(UUID tenantId);
