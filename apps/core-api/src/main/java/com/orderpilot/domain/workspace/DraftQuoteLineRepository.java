...
public interface DraftQuoteLineRepository extends JpaRepository<DraftQuoteLine, UUID> {
  ...
  // PR#236: bulk list-read loader — load lines for a bounded set of quote ids in one tenant-scoped
  // query (ordered for deterministic in-memory grouping), avoiding per-quote line loading in the
  // operator draft-quote list.
  List<DraftQuoteLine> findByTenantIdAndDraftQuoteIdInOrderByDraftQuoteIdAscLineNumberAsc(UUID tenantId, Collection<UUID> draftQuoteIds);
