...
public class IntegrationConnectionController {
  ...
  @PostMapping("/connections/{id}/health-check") public IntegrationHealthResponse health(@PathVariable UUID id) { var r = connectionService.recordHealthCheck(id); return new IntegrationHealthResponse(r.providerType().name(), r.healthy(), r.statusCode(), r.message()); }
  ...
  private IntegrationConnectionResponse toResponse(IntegrationConnection c) {
    return new IntegrationConnectionResponse(c.getId(), c.getProviderType().name(), c.getDisplayName(), c.getStatus(), c.getMode(), c.getConnectionKind(), c.getSecretRef() != null && !c.getSecretRef().isBlank(), c.getEndpointRef(), c.getLastSyncAt(), c.getLastHealthCheckAt(), c.getCreatedAt(), c.getUpdatedAt());
  }

  private ConnectorSyncEventResponse toResponse(ConnectorSyncEvent e) {
    return new ConnectorSyncEventResponse(e.getId(), e.getIntegrationConnectionId(), e.getProviderType().name(), e.getSyncType(), e.getDirection(), e.getStatus(), e.getRecordsRead(), e.getRecordsWritten(), e.getRecordsFailed(), e.getStartedAt(), e.getFinishedAt(), e.getErrorCode(), e.getErrorMessage());
  }
