...
public class IntakeMessageController {
  ...
  @GetMapping("/messages") public List<ChannelMessageResponse> list(@RequestParam(defaultValue = "50") int limit){ return service.list(limit).stream().map(this::toResponse).toList(); }
  @GetMapping("/messages/{id}") public ChannelMessageResponse get(@PathVariable UUID id){ return toResponse(service.get(id)); }
  @GetMapping("/conversations/{conversationId}") public List<ChannelMessageResponse> conversation(@PathVariable String conversationId, @RequestParam(defaultValue = "50") int limit){ return service.conversation(conversationId, limit).stream().map(this::toResponse).toList(); }
  @GetMapping("/events") public List<InboundEventResponse> events(@RequestParam(defaultValue = "50") int limit){ return eventService.listLedger(limit).stream().map(this::toEvent).toList(); }
