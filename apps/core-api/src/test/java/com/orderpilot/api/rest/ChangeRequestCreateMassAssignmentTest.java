...
class ChangeRequestCreateMassAssignmentTest {
  ...
  void maliciousAuthorityAndStateBodyFieldsCannotPreApproveOrPreExecuteChangeRequest() throws Exception {
    ...
    when(service.createChangeRequest(anyString(), anyString(), anyString(), anyString(), any(), anyString(), any(), any())).thenReturn(persisted);

    String maliciousBody = "{"
        + "\"targetSystem\":\"ONEC\",\"targetEntity\":\"ORDER\",\"requestedAction\":\"CREATE_ORDER\","
        + "\"sourceType\":\"QUOTE\",\"sourceId\":\"" + sourceId + "\",\"requestPayloadJson\":\"{}\",\"idempotencyKey\":\"key\","
    ...
    // The command the service received carries only the 8 business-intent arguments; there is no
    // status/approval/execution/tenant parameter for the body to bind to, and createdBy is the
    // trusted actor, never the spoofed one.
