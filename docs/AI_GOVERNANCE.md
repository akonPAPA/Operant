...
- Deterministic backend validation owns the final business decision. Phase 5A consumes advisory extraction results and produces validation outcomes, issues, substitute candidates, and approval requirements without creating final business records.
- Phase 5B may convert validation outcomes into operator review cases and internal draft quote/order preparation through typed backend services. This is still not AI authority and does not permit direct AI-created business records.
...
Phase 5A additionally requires review for low-confidence extraction lines, unknown UOM values, ambiguous product matches, customer-blocked substitutes, discount violations, and margin guardrail violations.

Phase 5B draft preparation rejects blocked substitutes, unresolved unknown UOM values, unresolved ambiguous product matches, and discount/margin/low-confidence cases that have not passed review approval. ERP and external writes remain out of scope.
